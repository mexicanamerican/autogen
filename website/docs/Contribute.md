...
`pre-commit run --all-files` to check if you meet the pre-commit requirements. If you use Windows (without WSL) and can't commit after installing pre-commit, you can run `pre-commit uninstall --all-files` to uninstall the hook. In WSL or Linux this is supposed to work.
